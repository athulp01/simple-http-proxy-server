...
int builtin_cmd(char **argv) 
{
    if(!strcmp(argv[0], "fg") || !strcmp(argv[0], "bg")) {
        ...
        if(kill(job->pid, SIGKILL)<0)
            unix_error("Cannot kill process");
...
void do_bgfg(char **argv) 
{
    char *endptr;
    if(argv[1] == NULL) {
        printf("Job id should succeed the builtin_cmd\n");
        return;
    }
    int jid = strtol(argv[1]+1, &endptr, 10), status = -1;
    if(errno == EINVAL || endptr == argv[1]+1) { 
        printf("Job id should succeed the builtin_cmd\n");
        return;
    }
    if(jid > maxjid(jobs)) {
...
void sigchld_handler(int sig) 
{
    ...
    while((cpid = waitpid(-1, &status, WNOHANG))>0) {
        if(!deletejob(jobs, cpid)) 
            unix_error("Cannot deletejob");
    }
    errno = olderr;
    return;
}
